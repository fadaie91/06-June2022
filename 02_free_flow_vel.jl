...
ax_vp = Axis(fig[1, 2], title=partial_cell_title)
ax_vf = Axis(fig[2, 2], title="GridFittedBottom")
ax_vd = Axis(fig[3, 2], title="Difference (GridFitted - PartialCell)")
...
Colorbar(fig[1, 1], hmvp, label="Velocity", flipaxis=false)

hmvf = heatmap!(ax_vf, v_full)
contour!(ax_vf, v_full; levels, color, linewidth)
Colorbar(fig[2, 1], hmvf, label="Velocity", flipaxis=false)
